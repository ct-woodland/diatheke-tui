...
# http://www.huge-man-linux.net/man1/diatheke.html
# http://crosswire.org/wiki/Frontends:Diatheke
###################################
...
case $CHOICE in
	...
	if (whiptail --title "Install Modules" --yes-button "Bible" --no-button "Dictionary"  --yesno "What type of module do you wish to download?" 10 60) then
		...
		GET_BIBLES=$(curl --silent "http://www.crosswire.org/sword/modules/ModDisp.jsp?modType=Bibles" 2>&1 | grep "SwordMod.Verify?modName=" |  sed -e 's/^.*\(modName=.*&pkgType\).*$/\1/' -e 's/modName=//' -e 's/&pkgType//')
		...
		exitstatus=$?
		if [ $exitstatus = 0 ]; then
			nohup curl "http://www.crosswire.org/ftpmirror/pub/sword/packages/rawzip/$BIBLE_MOD.zip" -o "$SWORD_DIR"/"$BIBLE_MOD".zip 2>&1 -# | stdbuf -oL tr '\r' '\n' | grep -o '[0-9]*\.' &> /tmp/module_down &
			{
			for ((i = 0 ; i <= 100 ; ++i)); do
		...
		GET_DICTS=$(curl --silent "http://www.crosswire.org/sword/modules/ModDisp.jsp?modType=Dictionaries" 2>&1 | grep "SwordMod.Verify?modName=" |  sed -e 's/^.*\(modName=.*&pkgType\).*$/\1/' -e 's/modName=//' -e 's/&pkgType//')
		## Whiptail menu of above CrossWire array
		...
		exitstatus=$?
		if [ $exitstatus = 0 ]; then
			nohup curl "http://www.crosswire.org/ftpmirror/pub/sword/packages/rawzip/$DICT_MOD.zip" -o "$SWORD_DIR"/"$DICT_MOD".zip 2>&1 -# | stdbuf -oL tr '\r' '\n' | grep -o '[0-9]*\.' &> /tmp/module_down &
			{
			for ((i = 0 ; i <= 100 ; ++i)); do
				## Make up "fake" percentage until curl actually reaches 100.
				if [[ $(tail -1 "/tmp/module_down") = "100." ]]
	...
	if (whiptail --title "Install Extra-Modules" --yes-button "Commentary" --no-button "Generic Book"  --yesno "What type of extra-module do you wish to download?" 10 60) then
		...
		GET_COMMEN=$(curl --silent "http://www.crosswire.org/sword/modules/ModDisp.jsp?modType=Commentaries" 2>&1 | grep "SwordMod.Verify?modName=" |  sed -e 's/^.*\(modName=.*&pkgType\).*$/\1/' -e 's/modName=//' -e 's/&pkgType//')
		## Whiptail menu of above CrossWire array
		...
		exitstatus=$?
		if [ $exitstatus = 0 ]; then
			nohup curl "http://www.crosswire.org/ftpmirror/pub/sword/packages/rawzip/$COMMEN_MOD.zip" -o "$SWORD_DIR"/"$COMMEN_MOD".zip 2>&1 -# | stdbuf -oL tr '\r' '\n' | grep -o '[0-9]*\.' &> /tmp/module_down &
			{
			for ((i = 0 ; i <= 100 ; ++i)); do
				## Make up "fake" percentage until curl actually reaches 100.
				if [[ $(tail -1 "/tmp/module_down") = "100." ]]
		...
		GET_BOOKS=$(curl --silent "http://www.crosswire.org/sword/modules/ModDisp.jsp?modType=Books" 2>&1 | grep "SwordMod.Verify?modName=" |  sed -e 's/^.*\(modName=.*&pkgType\).*$/\1/' -e 's/modName=//' -e 's/&pkgType//')
		## Whiptail menu of above CrossWire array
		...
		exitstatus=$?
		if [ $exitstatus = 0 ]; then
			nohup curl "http://www.crosswire.org/ftpmirror/pub/sword/packages/rawzip/$BOOK_MOD.zip" -o "$SWORD_DIR"/"$BOOK_MOD".zip 2>&1 -# | stdbuf -oL tr '\r' '\n' | grep -o '[0-9]*\.' &> /tmp/module_down &
			{
			for ((i = 0 ; i <= 100 ; ++i)); do
				## Make up "fake" percentage until curl actually reaches 100.
				if [[ $(tail -1 "/tmp/module_down") = "100." ]]
